...
&nbsp; - Properly checks `WALL\_MOVEMENT\_TYPES` instead of legacy wall sense types
...
\- Checks `wall.document.move` for `WALL\_MOVEMENT\_TYPES.NONE` to skip non-blocking walls
...
&nbsp; - Fixed wall movement type constant (using WALL\_MOVEMENT\_TYPES instead of WALL\_SENSE\_TYPES)
...
\- Wall detection uses correct Foundry v13 constants (WALL\_MOVEMENT\_TYPES)
...
\- Constants added: MAX\_PORTRAITS, PORTRAIT\_SIZE, NEARBY\_DISTANCE\_MULTIPLIER, SPIRAL\_SEARCH\_MAX\_RADIUS, WALL\_COLLISION\_TEST\_OFFSET
