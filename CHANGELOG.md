...
### Fixed
- **Function Exports**: Added `showSplitPartyDialog` and `splitAndDeployMembers` to `window.PartyVision` exports
  - These functions were defined but not globally accessible, causing the Split Party button to fail
  - Split Party button now properly appears in the Deploy Party dialog
  - Functions can now be called from both module scope and console for debugging

### Technical Notes
- ES module functions must be explicitly exported to `window` to be accessible from Dialog callbacks in Foundry VTT
- This ensures proper function resolution in asynchronous callback contexts
...
### Added
- **Split Party Feature**: New ability to split party tokens during deployment
  - Added "Split Party" button to deployment dialog for selective member deployment
  - Can deploy selected members individually or create a new party token from selection
  - Intelligent member selection UI with visual indicators and select all/deselect all
  - Automatic lighting transfer to split members and new party tokens
  - Smart placement: new party tokens appear adjacent to original, avoiding walls and tokens
  
- **Enhanced Deployment Dialog**: Completely redesigned deployment interface
  - Modern, cleaner styling with better visual hierarchy
  - Improved button layout with icons for better usability
  - "Deploy All" and "Split Party" options clearly separated
  - Better responsive design and hover effects
  - Consistent styling across all party-related dialogs

### Fixed
- **Deployment Directions**: Corrected rotation calculation for east, south, and west deployments
  - Removed erroneous negative sign in rotation angle calculation
  - North deployment: Leader correctly positioned north of formation ✓
  - East deployment: Leader now correctly positioned east of formation
  - South deployment: Leader now correctly positioned south of formation
  - West deployment: Leader now correctly positioned west of formation
  - Formation orientation now properly matches selected deployment direction

### Improved
- **Wall Collision Detection**: Optimized wall handling throughout entire module
  - Enhanced `isSpotValid()` function with better edge detection
  - New `isSpotValidForPlacement()` specifically for split party placement
  - More reliable collision testing using multiple test points per token
  - Better handling of `WALL_MOVEMENT_TYPES` for Foundry v13 compatibility
  
- **Adjacent Placement Logic**: New intelligent positioning system
  - `findAdjacentSpot()` function checks right, bottom, left, then top positions
  - Prioritizes clear spaces without walls or token overlaps
  - Falls back to spiral search if no adjacent space found
  - Prevents tokens from being created on top of each other
  
- **Lighting Management**: Enhanced light handling for split operations
  - Automatically transfers lighting from split members to deployed tokens
  - Updates original party token lighting when members with light sources leave
  - New party tokens inherit appropriate lighting from their members
  - Real-time lighting synchronization across all party operations

### Technical Details
- Added `showSplitPartyDialog()` for split member selection interface
- Added `splitAndDeployMembers()` for handling split logic and member management
- Added `createSplitPartyToken()` for creating new party tokens from selected members
- Added `deploySelectedMembers()` for deploying selected members individually
- Added `updatePartyToken()` for updating original token after split
- Added `findAdjacentSpot()` for intelligent placement of new tokens
- Added `isSpotValidForPlacement()` with exclude token parameter for split scenarios
- Improved CSS with new `.party-vision-split-dialog` styles
- Enhanced dialog buttons with icon support and better spacing
- Rotation angle calculation simplified: now `radians - lastFacing` (no negation)
- All split operations properly handle recentering of member positions
- Automatic leader assignment when split parties don't have designated leaders
